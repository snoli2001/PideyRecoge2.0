...
void main()
{
		...
		if (archi1.is_open()) {
			while (!archi1.eof())
			{
				archi1 >> nombre1;
				archi1 >> apellido1;
				Pedido* pe1 = new Pedido(nombre1, apellido1);
				archi1 >> con;
				archi1 >> inc;
				for (int i = 0; i < con; i++)
				{
					...
					pe1->AgregarAlimentoaLista_inicio(new Contable(producto, cant));
				...
				for (int i = 0; i < inc; i++)
				{
					...
					pe1->AgregarAlimentoaLista_inicio(new Incontable(producto, cant, medida));
				}
				proceso->Encolar(pe1);
			}
